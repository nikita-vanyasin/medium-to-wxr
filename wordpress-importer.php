	...
	function fetch_remote_file( $url, $post ) {
		...
		if ( $remote_response_code != '200' ) {
			...
			return new WP_Error(
                'import_file_error',
                sprintf(
                    __('Remote server returned error response %1$d %2$s %3$s', 'wordpress-importer'),
                        esc_html($remote_response_code),
                        get_status_header_desc($remote_response_code),
                        var_export($remote_response, true)
                )
            );
		}
		...
		{
            // get content type
            $contentType = isset($headers['content-type']) ? $headers['content-type'] : 'errUnknowwwn';

            $mimes = array_flip(get_allowed_mime_types());
            $availableExtensionsStr = isset($mimes[$contentType]) ? $mimes[$contentType] : '';
            $availableExtensions = explode('|', $availableExtensionsStr);
            if (count($availableExtensions) < 1 || empty($availableExtensions[0])) {
                @unlink( $upload['file'] );
                return new WP_Error(
                    'import_file_error',
                    sprintf(
                        __('unknown ext. Searched for: ' . $contentType, 'wordpress-importer')
                    )
                );
            }
            $ext = $availableExtensions[0];
            $newFileName = str_replace('.bmp', '.' . $ext, $upload['file']);
            rename($upload['file'], $newFileName);
            $upload['file'] = $newFileName;

            $upload['url'] = str_replace('.bmp', '.' . $ext, $upload['url']);
		}
